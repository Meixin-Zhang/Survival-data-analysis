...
model6.unadjust <- coxph(s.gvhd~mtx,data=data)
summary(model6.unadjust)
exp(confint.default(model6.unadjust,level = 0.90))

# adjusted model cox
model6.full <- coxph(s.gvhd~mtx+age+donorage+male+donormale+as.factor(disgroup),data=data)
summary(model6.full)
exp(confint.default(model6.full,level = 0.90))

# unadjusted model aft
weibull.aft<-flexsurvreg(s.gvhd~mtx,data=data,dist = "weibull")
weibull.aft
exp(confint.default(weibull.aft,level = 0.90))

#adjusted model aft
weibull.aft.adjust <- flexsurvreg(s.gvhd~mtx+age+donorage+male+donormale+as.factor(disgroup),data=data,dist = "weibull")
weibull.aft.adjust
exp(confint.default(weibull.aft.adjust,level = 0.90))
...
cox.bmt.tvc3.adjust<-coxph(surv.bmt.tvc3~postrnpl+age+cmv+
                             as.factor(disgroup)+fab, data = bmt.tvc3)
cox.bmt.tvc3.adjust
...
cox.bmt.tvc4.adjust<-coxph(surv.bmt.tvc4~postrnpl+age+cmv+
                             as.factor(disgroup)+fab, data = bmt.tvc4)
cox.bmt.tvc4.adjust
